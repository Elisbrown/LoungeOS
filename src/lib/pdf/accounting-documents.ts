...
async function addUniversalHeaderAndFooter(doc: jsPDF, title: string, userInfo?: { name: string }, settings?: Settings) {
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        doc.text(`Date: ${new Date().toLocaleString()}`, rightX, 17, { align: 'right' });
...
export async function generateJournalEntryPDF(entry: any, settings: Settings, userInfo?: { name: string }) {
  const doc = new jsPDF();
  
  // Entry Details
  doc.setFontSize(10);
  doc.text(`Entry #: ${entry.id}`, 14, 45);
  doc.text(`Date: ${new Date(entry.entry_date).toLocaleDateString()}`, 14, 51);
  doc.text(`Type: ${entry.entry_type}`, 14, 57);
  doc.text(`Reference: ${entry.reference || 'N/A'}`, 14, 63);
  doc.text(`Description: ${entry.description}`, 14, 69);
  ...
  await addUniversalHeaderAndFooter(doc, 'Journal Entry', userInfo, settings);
  
  return doc;
}

// Generate Profit & Loss PDF
export async function generateProfitLossPDF(report: any, settings: Settings, userInfo?: { name: string }) {
  const doc = new jsPDF();
  
  doc.setFontSize(10);
  const periodStartY = 45;
  doc.text(`Period: ${new Date(report.period.start).toLocaleDateString()} - ${new Date(report.period.end).toLocaleDateString()}`, 14, periodStartY);
  ...
  if (revenueData.length > 0) {
    doc.setFontSize(12);
    doc.text('Revenue', 14, periodStartY + 10);
  ...
  await addUniversalHeaderAndFooter(doc, 'Profit & Loss Statement', userInfo, settings);
  
  return doc;
}

// Generate Balance Sheet PDF
export async function generateBalanceSheetPDF(report: any, settings: Settings, userInfo?: { name: string }) {
  const doc = new jsPDF();
  
  doc.setFontSize(10);
  const startY = 45;
  doc.text(`As of: ${new Date(report.asOfDate).toLocaleDateString()}`, 14, startY);
  ...
  await addUniversalHeaderAndFooter(doc, 'Balance Sheet', userInfo, settings);
  
  return doc;
}

// Generate Cash Flow PDF
export async function generateCashFlowPDF(report: any, settings: Settings, userInfo?: { name: string }) {
  const doc = new jsPDF();
  
  doc.setFontSize(10);
  const startY = 45;
  doc.text(`Period: ${new Date(report.period.start).toLocaleDateString()} - ${new Date(report.period.end).toLocaleDateString()}`, 14, startY);
  ...
  const summaryData = [
    ...
    [{ content: 'Ending Cash', styles: { fontStyle: 'bold' as any } }, { content: formatAmount(report.endingCash, settings.defaultCurrency), styles: { fontStyle: 'bold' as any } }],
  ];
  ...
  await addUniversalHeaderAndFooter(doc, 'Cash Flow Statement', userInfo, settings);
