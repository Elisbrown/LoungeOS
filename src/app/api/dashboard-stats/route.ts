...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const categorySalesData = fromDate && toDate
      ? categorySalesStmt.all(...dateParams) as { category: string, sales: number }[]
      : categorySalesStmt.all() as { category: string, sales: number }[];

    // Map ALL categories, even those with zero sales
    const categorySales = allCategories.map((cat, index) => {
      const saleEntry = categorySalesData.find(s => s.category.toLowerCase() === cat.name.toLowerCase());
      return {
        category: cat.name,
        sales: saleEntry ? saleEntry.sales : 0,
    ...
    const productSalesData = fromDate && toDate
      ? productSalesStmt.all(...dateParams) as { label: string, value: number }[]
      : productSalesStmt.all() as { label: string, value: number }[];
    ...
    const staffPerformanceCalculated = waiters.map(staff => {
      ...
      const prevStatsStmt = db.prepare(`
        SELECT 
          ...
          COUNT(*) as orders_processed
        FROM orders
        WHERE waiter_id = ? AND timestamp BETWEEN ? AND ?
      `)
