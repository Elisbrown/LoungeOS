...
function PosPageContent() {
  ...
  // Track if we've initialized for the current table to avoid resetting cart on order polling
  const [hasInitializedTable, setHasInitializedTable] = useState<string | null>(null);
  const [isInitialized, setIsInitialized] = useState(false);

  // On mount: restore table from URL param or sessionStorage
  useEffect(() => {
    if (isInitialized) return;

    const tableFromUrl = searchParams.get('table');
    const storedTable = typeof window !== 'undefined' ? sessionStorage.getItem('pos_selected_table') : null;

    if (tableFromUrl) {
      setSelectedTable(tableFromUrl);
      if (typeof window !== 'undefined') {
        sessionStorage.setItem('pos_selected_table', tableFromUrl);
      }
    } else if (storedTable) {
  ...
  // Load existing order for the selected table (only on table change, not on every poll)
  useEffect(() => {
    if (!isInitialized) return;

    if (selectedTable && selectedTable !== hasInitializedTable) {
      const activeOrder = orders.find(o => o.table === selectedTable && o.status !== "Completed" && o.status !== "Canceled");
      if (activeOrder) {
        setOrderItems(activeOrder.items.map(i => ({ ...i, isPersisted: true, persistedQuantity: i.quantity })));
        setActiveOrderId(activeOrder.id);
      } else {
        setOrderItems([]);
        setActiveOrderId(null);
      }
      setHasInitializedTable(selectedTable);

      // Persist to sessionStorage whenever table changes
      if (typeof window !== 'undefined' && selectedTable) {
        sessionStorage.setItem('pos_selected_table', selectedTable);
      }
    }
  }, [selectedTable, orders, hasInitializedTable, isInitialized]);

  const availableTables = useMemo(() => {
    // First filter by status
    let filtered = tables.filter(table => table.status === 'Available' || table.status === 'Occupied');

    // Then filter by user's assigned floor (unless Manager/Super Admin or no floor assignment)
    if (user && user.role !== "Manager" && user.role !== "Super Admin") {
      if (user.floor && user.floor !== "All" && user.floor !== "") {
        filtered = filtered.filter(table => table.floor === user.floor);
      }
    }
  ...
  const handleTableSelect = (tableName: string) => {
    ...
    setHasInitializedTable(null); // Reset so useEffect can load the order for this table
  };
    ...
    <>
      <div className="flex h-screen w-full flex-col">
        ...
        <main className="flex flex-1 flex-col md:flex-row overflow-hidden">
          {/* Products Grid - takes full width on mobile, flexible on desktop */}
          <div className="flex-1 overflow-y-auto p-3 md:p-4 lg:p-6">
            <ProductGrid onProductClick={handleProductClick} />
          </div>
          {/* Order Summary / Cart - full width on mobile, fixed width on desktop */}
          <div className="w-full md:w-80 lg:w-96 border-t md:border-t-0 md:border-l bg-card flex flex-col h-auto md:h-full overflow-hidden">
            <div className="flex-none p-3 md:p-4 border-b">
              <div className="flex items-center justify-between">
                <Button variant="outline" size="sm" onClick={() => setSelectedTable('')}>
                  <ArrowLeft className="mr-2 h-4 w-4" />
                  <span className="hidden sm:inline">{t('pos.changeTable')}</span>
                </Button>
                <div className="text-right">
                  <Label htmlFor="table-select" className="text-[10px] uppercase font-bold text-muted-foreground">{t('pos.currentTable')}</Label>
                  <h2 id="table-select" className="text-lg md:text-xl font-black text-primary">{selectedTable}</h2>
                </div>
              </div>
            </div>
